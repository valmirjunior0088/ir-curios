...
translateFunction scope = do
  ...
  let
    ...
    variables = filter (/= parameter) (free output)
...
translateTerm = \case
  ...
  Core.Local variable -> translateLocal variable
  Core.Type -> translateNull
  Core.FunctionType _ _ -> translateNull
  Core.Function scope -> translateFunction scope
  Core.Apply function argument -> translateApply function argument
  Core.PairType _ _ -> translateNull
  Core.Pair left right -> translatePair left right
  Core.Split scrutinee scope -> translateSplit scrutinee scope
